...
class WikiData:
    ...
    def get_individu(self, wikidata_id, n=1):
        ...
        obj = self.client.get(f'Q{wikidata_id}', load=True)
        self.logger.warning(f'{n}\thttps://www.wikidata.org/wiki/Q{wikidata_id}   \t{obj.description}')
        ...
        if 'P22' in claims and 'datavalue' in claims['P22'][0]['mainsnak']:
            mari, _ = Individu.objects.get_or_create(wikidata=self.get_id(claims['P22'][0]))
            family.append(self.get_id(claims['P22'][0]))
        if 'P25' in claims and 'datavalue' in claims['P25'][0]['mainsnak']:
            femme, _ = Individu.objects.get_or_create(wikidata=self.get_id(claims['P25'][0]))
